...
public class TestCase {
    ...
    @Test(description = "学习报告正向用例")
    public void LearningReport() throws IOException{
    ...
    @Test(description = "学习报告错误用例")
    public void test2() throws IOException{
        DataDao dataDao=new DataDao();
        JSONObject jsonObject=JSON.parseObject(dataDao.selectTestDataListById("3").get(0).getRequest());
        JSONObject responseData=JSON.parseObject(dataDao.selectTestDataListById("3").get(0).getResponse());
        String response =controller.getHttpClient().Post(jsonObject, ConfigFile.getUrl(STUDYREPORT),"crmadmin");
        JSONObject responseJson = JSONObject.parseObject(response);
        String code =responseJson.getString("code");
        String expectedCode=responseData.getString("code");
